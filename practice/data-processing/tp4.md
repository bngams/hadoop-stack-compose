...
## Outils et Versions
* [Apache HBase](https://hbase.apache.org/) Version 2.5.x (ou alternative NoSQL)
...
??? warning "HBase n'est pas inclus dans l'architecture de base"
    **Important**: HBase n'est pas actuellement configuré dans le fichier [docker-compose.yml](../../docker-compose.yml). Vous avez plusieurs options:

    **Option 1 - Utiliser Hive à la place de HBase** (déjà disponible):

    Votre architecture inclut déjà Hive qui est configuré et prêt à l'emploi. Hive est une alternative pour le stockage et le traitement de données structurées sur HDFS. Voir la section Hive dans votre docker-compose.yml:
    - HiveServer2 disponible sur `localhost:10000`
    - Interface Web Hive sur `localhost:10002`

    **Option 2 - Ajouter HBase au docker-compose.yml**:
    ...
    **Option 3 - Installation manuelle dans le contenaire namenode**:
    ...
    # Configurer hbase-site.xml avec HDFS
    # Puis démarrer HBase
    start-hbase.sh
    ```

Si vous utilisez HBase, suivre les étapes décrites dans la partie _Installation_ du [TP1](tp1.md#installation) pour lancer le cluster Hadoop:

```bash
docker compose up -d
```

Puis entrer dans le contenaire approprié:

```bash
docker exec -it namenode bash  # ou hbase-master si vous avez ajouté le service
```

En tapant `jps`, vous devriez voir les processus HBase (si installé):
```bash
161 NameNode
1138 HRegionServer
1028 HMaster
...
```bash
docker cp target/processing-1.jar namenode:/root/
```
