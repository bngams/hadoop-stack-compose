...
## Outils et Versions
* [Apache Kafka](https://kafka.apache.org/) Version 3.6.1
...
??? warning "Kafka n'est pas inclus dans l'architecture de base"
    **Important**: Kafka n'est pas actuellement configuré dans le fichier [docker-compose.yml](../../docker-compose.yml). Vous devez soit:

    **Option 1 - Installation manuelle dans un contenaire**:

    Installer Kafka manuellement dans le contenaire namenode:
    ...
    # Démarrer Zookeeper (requis pour Kafka)
    ...
    **Option 2 - Ajouter Kafka au docker-compose.yml** (recommandé):

    Ajouter les services suivants à votre fichier docker-compose.yml:
...
Si vous utilisez l'Option 1, suivre les étapes décrites dans la partie _Installation_ du [TP1](tp1.md#installation) pour lancer le cluster Hadoop:
...
Vérifier que Kafka et Zookeeper sont lancés en tapant `jps`. Vous devriez voir (en plus des processus Hadoop):
```bash
2756 Kafka
53 QuorumPeerMain
```
...
### Producteur
 ...
 Pour créer un producteur Kafka, créer un fichier dans un répertoire de votre choix dans le contenaire namenode (ou dans le contenaire kafka si vous utilisez l'Option 2), intitulé ```SimpleProducer.java```. Son code est le suivant:
...
Dans le répertoire target, un fichier ```stream-kafka-spark-1-jar-with-dependencies.jar``` est créé. Copier ce fichier dans le contenaire namenode, en utilisant le terminal comme suit:

```bash
docker cp target/stream-kafka-spark-1-jar-with-dependencies.jar namenode:/root
```

Revenir à votre contenaire namenode, et lancer la commande ```spark-submit``` pour lancer l'écouteur de streaming spark.
